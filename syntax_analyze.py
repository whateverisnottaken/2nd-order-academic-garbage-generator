...
class SyntaxAnalyze(object):
    ...
    def run_on_lr_dfa(self, tokens):
        ...
        for token in tokens:
            ...
            self.indx_to_token.append(token)
            self.grammar_tree.append(cur_index)
            token_index.append(cur_index)
        tokens.reverse()
        token_index.reverse()
        #print tokens
        #cur_index = cur_index + 1
        #self.indx_to_token.append(tokens[-1])
        #self.grammar_tree.append(cur_index)
        while not success:
            top = status_stack[-1]
            ...
            if tokens[-1] in self.lr_analyze_table[top]:
                ...
                if action[0] == 's':
                    ...
                    index_stack.append(token_index[-1])
                    tokens = tokens[:-1]
                    ...
                    tokens.append(left)
                    cur_index = cur_index + 1
                    self.indx_to_token.append(left)
                    self.grammar_tree.append(cur_index)
                    token_index.append(cur_index)
                    if production[left] == ['$']:
                        continue
                    for i in range(len(production[left])):
                        now_index=index_stack[-1-i]
                        if (not self.indx_to_token[now_index-1] in production[left]):
                            print("WTF")
                        self.grammar_tree[now_index-1]=cur_index
                    status_stack = status_stack[:-right_len]
                    symbol_stack = symbol_stack[:-right_len]
                    index_stack = index_stack[:-right_len]
                    #print symbol_stack
                    #print '\n'
                else:
                    status_stack.append(action[1])
    ...
    def print_grammar_tree(self,now,deep):
        output_str=""
        for i in range(2*deep):
            output_str=output_str+" "
        output_str=output_str+self.indx_to_token[now-1]
        print output_str
        for i in range(len(self.indx_to_token)):
            if (i+1!=now and self.grammar_tree[i]==now):
                self.print_grammar_tree(i+1,deep+1)
...
def main():
    ...
    output=open("ana_table.txt","w")
    for key in syn_ana.lr_analyze_table:
        output.write(str(key)+ ': '+ str(syn_ana.lr_analyze_table[key]))
    for i in range(len(syn_ana.grammar_tree)):
        if (i+1==syn_ana.grammar_tree[i]):
            syn_ana.print_grammar_tree(i+1,0)
